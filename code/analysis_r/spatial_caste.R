...

{
  ...
  library(stargazer)
  x <- c("ggplot2",  "foreign", "xtable", "splitstackshape", "dplyr", "tidyr", "zoo",
...
conflict <- read.csv("./Data/ACLED/acled_india_castes.csv", sep=";") ## india_new.csv ## acled_india_3: Caste stuff
...
# Regressions for main text and poisson regressions for appendix
{
  ...
  emodel <-  acled_totalevents ~ sc_reserved + sc_prop_01_norm + factor(ST_NAME) +  
    pc01_pca_tot_p_norm + 
    pc01_vd_p_sch_norm + 
    pc01_vd_tar_road_norm 
  fmodel <-  fat ~ sc_reserved + sc_prop_01_norm + factor(ST_NAME) +
    pc01_pca_tot_p_norm +
    pc01_vd_p_sch_norm + 
    pc01_vd_tar_road_norm 
  logmodel <- log(acled_totalevents+1) ~ sc_reserved + sc_prop_01_norm +  factor(ST_NAME) +
    pc01_pca_tot_p_norm + 
    pc01_vd_p_sch_norm + 
    pc01_vd_tar_road_norm 
  logmodelf <- log(fat+1) ~ sc_reserved + sc_prop_01_norm +  factor(ST_NAME) +
    pc01_pca_tot_p_norm +
    pc01_vd_p_sch_norm + 
    pc01_vd_tar_road_norm 
  ...
  stargazer(ols11, ols1,  nb11, nb1, ps11, ps1,
            title = "Robustness Check: Constituency assemblies, caste-related conflict only",
            covariate.labels = c("\\textbf{Seat reserved for SC}",
                                 ...
                                 "Constant"),
            no.space=TRUE,
            column.labels   = c("OLS: log(Events+1)", "NB: Events", "Poisson: Events"),
            column.separate = c(2, 2, 2),
            model.names = F,
            dep.var.caption = "",
            dep.var.labels.include = F,
            omit.stat=c("ser", "theta", "f" ),
            notes = c("Results from regressions of conflict events/fatalities for assemblies.",
                      "Only caste-related conflict events included (n=3,428).",
                      "Signficance levels: $^*10\\%, ^{**}5\\%, ^{***}1\\%$.\\label{tab:assemblyc}"),
            notes.align = "l",
            notes.append=F,
            notes.label = "\\textbf{Note:}",
            out="./Output/tables/assemblyreg_c.tex")
...
# Make predicted value graphs for main text
{
  ...
  make_prediction <- function(df, model, type) {
    ...
    graph <-  ggplot(pr, aes(sc_prop_01_norm*v1+m1, p_events, group=sc_reserved))  +
      ...
      theme_minimal() +
      coord_cartesian(xlim=c(12,32), ylim = c(0,0.5)) +
  ...
  print(g1)
  tikz(file="./Output/figures/nb_events_c.tex", standAlone=F,
       width = 5, height = 4)
  print(g1)
  dev.off()
  ...
  tikz(file="./Output/figures/ps_events_c.tex", standAlone=F,
       width = 5, height = 4)
  print(g5)
  dev.off()
  ...
  tikz(file="./Output/figures/log_events_c.tex", standAlone=F,
       width = 5, height = 4)
  print(g7)
  dev.off()
