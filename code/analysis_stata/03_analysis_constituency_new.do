...
save "${gen}/delimitations_all_const.dta", replace
use "${gen}/delimitations_all_const.dta", replace
...
// import delim using "${data}/ACLED/acled_india_castes.csv", clear delim(";")
...
gen one = 1
gen protest = (event_type=="Protests")
gen riots = (event_type=="Riots")
gen battles = (event_type=="Battles")
gen repression = (event_type=="Violence against civilians")
gen remote = (event_type=="Explosions/Remote violence")

gen peaceful_protest = (sub_event_type=="Peaceful protest")
gen violent_protest = (sub_event_type=="Violent demonstration")
gen protest_with_intervention = (sub_event_type=="Protest with intervention")

gen mob_violence = (sub_event_type=="Mob violence")
gen armed_clash = (sub_event_type=="Armed clash")
gen attack = (sub_event_type=="Attack")
gen ied = (sub_event_type=="Remote explosive/landmine/IED")

gen military_india = 0
replace military_india=1 if substr(actor1, 1, 21)=="Military Forces of In"
replace military_india=1 if substr(actor1, 1, 19)=="Police Forces of In"
replace military_india=1 if substr(actor2, 1, 21)=="Military Forces of In"
replace military_india=1 if substr(actor2, 1, 21)=="Military Forces of In"

gen nax_india = 0
replace nax_india = 1 if substr(actor1, 1, 3)=="CPI"
replace nax_india = 1 if substr(actor1, 1, 3)=="Nax"
replace nax_india = 1 if substr(actor2, 1, 3)=="CPI"
replace nax_india = 1 if substr(actor2, 1, 3)=="Nax"

gen civilians_india = 0
replace civilians_india = 1 if substr(actor1, 1, 4)=="Civi"
replace civilians_india = 1 if substr(actor2, 1, 4)=="Civi"
...
ren _ID id_ac
collapse (sum) fatalities one-civilians_india  , by(id_ac)
...
foreach x of varlist protest-civilians_india {
...
save "${gen}/constituency/dataset_clean.dta", replace
...
local c = 0
foreach eventtype in protest peaceful_protest violent_protest protest_with_intervention riots battles  {
    local c=`c'+1
    cap gen l`eventtype'=log(`eventtype'+1)
    reghdfe l`eventtype' ///
    sc_reserved sc_prop_01_norm ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1   ///
    ,  absorb(district_crosswalk) vce(cluster district_crosswalk)

    qui sum `eventtype' if e(sample)
...
using "${output}/tables/reg_assembly_types.tex" , style(tex)  
wrap varwidth(52) 
varlabels(sc_reserved " \rowcolor{LSEred!20}  \textbf{Seat reserved for SC} \rowcolor{LSEred!20}" sc_prop_01_norm "Perc. SC pop (std.)"
  )
eqlabels("", none) 
keep(sc_reserved sc_prop_01_norm)
order(sc_reserved sc_prop_01_norm)
    cells(b(star fmt(%9.3f)) se(par)) 
 hlinechar("{hline @1}")
stats(lmean N r2  , fmt(%9.2fc %9.0fc %9.3fc) labels("\midrule \addlinespace Mean of DV" "Observations" "R-squared"))
...
use "${gen}/constituency/dataset_clean.dta", replace

cap gen levents = log(acled_totalevents+1)
cap gen lfat = log(acled_fatalities+1)
...
reghdfe levents ///
    ...
    if sample_2 == 1 ///
    , absorb(district_crosswalk) vce(cluster district_crosswalk) 
qui sum acled_totalevents if e(sample)
estadd scalar lmean = `r(mean)'
...
reghdfe lfat ///
    ...
    if sample_2 == 1 ///
    , absorb(district_crosswalk)  vce(cluster district_crosswalk) 
qui sum acled_fatalities if e(sample)
estadd scalar lmean = `r(mean)'
estadd local dfe = "$\checkmark$"
...
cap encode district_crosswalk, gen(dcode)

nbreg acled_totalevents ///
    i.dcode sc_reserved sc_prop_01_norm ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1 ///
    ,   vce(cluster district_crosswalk)  
qui sum acled_totalevents if e(sample)
...
#delimit ;
estout 
ols1 ols2 poisson1 poisson2 nb1 nb2
using "${output}/tables/reg_assembly_stata.tex" , style(tex)  
wrap varwidth(52) 
varlabels(sc_reserved " \rowcolor{LSEred!20}  \textbf{Seat reserved for SC} \rowcolor{LSEred!20}" sc_prop_01_norm "Perc. SC pop (std.)"
  )
eqlabels("", none) 
...
order(sc_reserved sc_prop_01_norm)
    cells(b(star fmt(%9.3f)) se(par)) 
 hlinechar("{hline @1}")
stats(lmean N r2   , fmt(%9.2fc %9.0fc %9.3fc %9.3fc) labels("\midrule \addlinespace Mean of DV" "Observations" "R-squared" ))
starlevels(* 0.1 ** 0.05 *** 0.01)
nolabel replace collabels(none) mlabels(none)
note("\bottomrule")
  ; 
...
reghdfe levents ///
    sc_reserved c.sc_reserved#c.total_nax_event sc_prop_01_norm ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1 ///
    , absorb(district_crosswalk) vce(cluster district_crosswalk) 
qui sum acled_totalevents if e(sample)
...
nbreg acled_totalevents ///
    i.dcode sc_reserved c.sc_reserved#c.total_nax_event sc_prop_01_norm ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1 ///
    ,   vce(cluster district_crosswalk)  
qui sum acled_totalevents if e(sample)
...
nbreg acled_fatalities ///
    i.st_code sc_reserved c.sc_reserved#c.total_nax_event sc_prop_01_norm ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1 ///
    ,   vce(cluster district_crosswalk) 
qui sum acled_fatalities if e(sample)
...
estout 
ols1 ols2 poisson1 poisson2 nb1 nb2
using "${output}/tables/reg_assembly_nax.tex" , style(tex)  
wrap varwidth(68) 
varlabels(sc_reserved "Seat reserved for SC" c.sc_reserved#c.total_nax_event " \rowcolor{LSEred!20}  \textbf{Reserved $\times$ Naxalite intensity}   \rowcolor{LSEred!20}" sc_prop_01_norm "Perc. SC pop (std.)"
  )
eqlabels("", none) 
keep(sc_reserved c.sc_reserved#c.total_nax_event sc_prop_01_norm)
order(sc_reserved c.sc_reserved#c.total_nax_event sc_prop_01_norm)
    cells(b(star fmt(%9.3f)) se(par)) 
 hlinechar("{hline @1}")
stats(lmean N r2   , fmt(%9.2fc %9.0fc %9.3fc %9.3fc) labels("\midrule \addlinespace Mean of DV" "Observations" "R-squared"))
starlevels(* 0.1 ** 0.05 *** 0.01)
...
/*----------------------------------------------------*/
   /* [>   Caste-based conflict events ONLY   <] */ 
/*----------------------------------------------------*/
...
local c = 0
foreach eventtype in acled_totalevents protest peaceful_protest violent_protest riots mob_violence  {
    local c=`c'+1
    cap drop l`eventtype'
    cap gen l`eventtype'=log(`eventtype'+1)
...
estout 
reg1 reg2 reg3 reg4 reg5 reg6 
using "${output}/tables/reg_assembly_types_caste.tex" , style(tex)  
wrap varwidth(52) 
varlabels(sc_reserved " \rowcolor{LSEred!20}  \textbf{Seat reserved for SC} \rowcolor{LSEred!20}" sc_prop_01_norm "Perc. SC pop (std.)"
  )
eqlabels("", none) 
...
reghdfe protest_with_intervention ///
    sc_reserved sc_prop_01_norm ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1    ///
    ,  absorb(district_crosswalk)  ///
    vce(cluster district_crosswalk) 
...
encode district_crosswalk, gen(dcode)
...
estout 
ols1 ols2 poisson1 poisson2 nb1 nb2
using "${output}/tables/reg_assembly_caste_stata.tex" , style(tex)  
wrap varwidth(52) 
varlabels(sc_reserved "\rowcolor{LSEred!20}  \textbf{Seat reserved for SC}   \rowcolor{LSEred!20}" sc_prop_01_norm "Perc. SC pop (std.)"
  )
eqlabels("", none) 
...
order(sc_reserved sc_prop_01_norm)
    cells(b(star fmt(%9.3f)) se(par)) 
 hlinechar("{hline @1}")
stats(lmean N r2 dfe  , fmt(%9.2fc %9.0fc %9.3fc %9.3fc) labels("\midrule \addlinespace Mean of DV" "Observations" "R-squared" "District FE + cluster SE"))
starlevels(* 0.1 ** 0.05 *** 0.01)
nolabel replace collabels(none) mlabels(none)
note("\bottomrule")
  ; 
...
import delim using "${data}/ACLED/acled_india_nocastes.csv", clear delim(";")
keep if geo_precision == 1 | geo_precision==2
...
estout 
ols1 ols2 poisson1 poisson2 nb1 nb2
using "${output}/tables/reg_assembly_nocaste_stata.tex" , style(tex)  
wrap varwidth(52) 
varlabels(sc_reserved "\rowcolor{LSEred!20}  \textbf{Seat reserved for SC}   \rowcolor{LSEred!20}" sc_prop_01_norm "Perc. SC pop (std.)"
  )
eqlabels("", none) 
...
gen lfat = log(acled_fatalities+1)
...
ppmlhdfe acled_fatalities ///
 ...
 local orig1 = _b[sc_reserved]

/* [> Randomization inference: <] */ 
keep if sample_2 == 1 
bys district_crosswalk: egen targetseats = total(sc_reserved)
preserve
...
forv i = 1/3 {
    bys district_crosswalk: egen currentseats = total(reserved_ri)
    qui sum random 
    gen rmax = r(max)
    replace reserved_ri = 1 if rmax==random & currentseats<targetseats
    replace random = . if random==rmax
    drop currentseats rmax
    }


ppmlhdfe acled_fatalities ///
    reserved_ri sc_prop_01_norm ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1 ///
    , absorb(district_crosswalk)  vce(cluster district_crosswalk)
...
local coef1 = _b[reserved_ri]
}
di "`r', `coef1'"
qui use `ri', clear
qui replace coef1 = `coef1' if _n==`r'
...
hist coef1, frac xline(`orig1', lwidth(medthick)) xti("Coefficients from randomized reallocation of leftover seats") ti("Randomization inference") // `orig1'// 
graph export "${output}/figures/ri1assembly.pdf", replace


count 
local n = r(N)
count if coef1<=`orig1' 
local p1 =r(N)/`n'

di "RI p-values is `p1'."
stop
...
reghdfe levents ///
    sc_reserved  prop_sc prop_schools ///
    pc01_pca_tot_p_norm  pc01_vd_tar_road_norm ///
    if sample_2 == 1 ///
    , absorb(district_crosswalk)  ///
    vce(cluster district_crosswalk) 
sum acled_totalevents if e(sample)
...
gsort district_crosswalk -perc_sc

gen cutoff = perc_sc if sc_reserved==1 & sc_reserved[_n+1]==0 & district_crosswalk==district_crosswalk[_n+1]
bys district_crosswalk: egen c = max(cutoff)

gen runningvar = perc_sc-c

br ac_name district_crosswalk sc_reserved perc_sc cutoff c runningvar




reghdfe levents ///
    sc_reserved runningvar ///
    pc01_pca_tot_p_norm pc01_vd_p_sch_norm pc01_vd_tar_road_norm ///
    if sample_2 == 1 ///
    , vce(robust) 

rdrobust acled_totalevents runningvar, deriv(1) p(2)
rdplot acled_totalevents runningvar, deriv(1) p(2)
